...
| Q2 | Database | **Self-hosted Postgres on `jeraaiboss`, Pulse gets its OWN database.** ⚠️ **Open conflict (see §Database):** the Intacct toolkit runs on a *different* server (Hetzner 159.69.216.113), so "all products on one database" isn't automatic — needs a call. Review recommends a **dedicated Postgres instance for Pulse**, not a shared cluster. |
...
| Q4 | Role mapping | **Roles managed inside Pulse** (`employees.role`, admin-edited). M365 = authN only; Pulse = authZ. Source of truth for the permission matrix is `docs/prototype/HANDOFF.md` §2; `capabilities.ts` and RLS must both agree with it. **Offer:** Claude maps Ryan's organogram → `employees.manager_id` hierarchy + initial role seed. |
...
**The host conflict to resolve first:** your Intacct toolkit's Postgres is on **Hetzner (159.69.216.113)**; Pulse deploys to **jeraaiboss (154.70.249.26)**. They are different machines. "Same database" would therefore require either (a) a cross-server DB connection (discouraged — latency + a new network attack path to finance data), or (b) consolidating the toolkit onto `jeraaiboss`. **Decision needed (Ryan):** confirm whether the toolkit stays on Hetzner (→ Pulse gets its own independent Postgres on `jeraaiboss`, the simplest, safest option) or is being consolidated.
